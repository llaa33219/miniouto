...
Why no `tool_result` reaction: a 400/422 about tool calls means the poison is IN the request history, so appending a synthetic result and resending the same payload loops forever (coreouto `examples/29_malformed_tool_call.py` — each repetition also piles a duplicate result onto the same tool call, its own 400 reason). These lists use `reaction="retry"` (retry_max=2, zero delay) for every tool-call 400/422 instead, paired with `core/runtime.py:_make_history_repair_hook` — the hook fires before each retry attempt and repairs the live message list in place (pair/dedupe first, then cut the poisoned turn), so the retry actually reaches the model. If both repair passes fail, the exception raises and the turn ends.
...
8. **Build `call_subagent` tool** — Via `_build_subagent_tool("subagent", description=_subagent_description(), provider_config=subagent_provider_config)`. The handler is pre-wrapped with `_wrap_subagent_handler` to track depth. Registered via `co.register_tool(name, description=description)(wrapped_handler)` (function-call form, not decorator).
 9. **Register hooks:**
   ...
   - `ON_THINKING` → `_make_thinking_logger(on_thinking)` (only if `on_thinking` is not None — `chat.run_chat` always supplies one).
    ...
    - `ON_PROVIDER_ERROR` → `_make_history_repair_hook()` (**always**, unlike the sink loggers) — repairs the live message list in place before each retry attempt of the 400/422 tool-call rules in `core/error_rules.py`. First fire runs `_repair_pairing` (drop duplicate tool results, fill dangling tool calls on the last assistant turn with error results); if that changed nothing — or the retry 400s again — `_cut_poisoned_turn` excises the malformed assistant tool-call turn plus its results and appends a user message explaining the removal, so the model continues with context instead of repeating the call (coreouto `examples/29_malformed_tool_call.py`). Gated to `reaction == "retry"` with status 400/422; rate-limit/overload retries pass through untouched. Pass counts are keyed by `id(messages)` inside the hook closure, which `build_runtime` rebuilds every turn.
