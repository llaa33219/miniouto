...
Idempotent: calls `_register_if_missing(name, handler, schema, description)` for `Bash`, `Image`, `Video`, `Audio`, and — when `computer_supported()`, with the handler variant chosen by the outo provider's `api_format` (screenshot-degraded on `openai`) — `Computer` (with `parallelizable=False`). (The `call_subagent` tool is registered separately in `core.runtime.build_runtime` because it needs the subagent config to be built first.)
