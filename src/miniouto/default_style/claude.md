...
- All file work goes through Bash: read with `cat`/`grep`/`find`, write
  with `cat > file <<'EOF'` or `tee`, edit with `sed -i` or a short
  Python snippet, delete with `rm`.
...
- **Bash** — shell command, 1-hour timeout, output truncated at
