...
- **Provider-format gate**: `register_all(api_format=...)` skips the Computer tool entirely when the outo provider's `api_format == "openai"` (Chat Completions) — that format rejects multimodal tool results, so a `screenshot` would raise `ValueError` and kill the whole turn mid-flight. anthropic / openai-response / google all accept image tool results. On gated combinations the model gets coreouto's unknown-tool error instead — same teaching-signal philosophy as the platform gate.
...
- The bundled styles still mention Computer unconditionally; on unsupported platforms or gated provider formats a stray model call to it fails with coreouto's unknown-tool error, which is an acceptable teaching signal.
