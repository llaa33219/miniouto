...
- **Screenshot degradation by provider format**: `register_all(api_format=...)` passes the outo provider's format into `_make_computer_handler`. openai (Chat Completions) rejects multimodal tool results while the provider formats the request, so on that format `screenshot` returns an explicit notice ("this model runs on an openai chat-completions provider, cannot view the screen — relay to the user; GUI actions still work") instead of raising `ValueError` and killing the turn. anthropic / openai-response / google return the image as always. The registered handler annotates `action`/`scroll_direction` with Literals — safe since coreouto 0.11.3 emits `type` for them (type-less enums before that were rejected by strict validators with a 400, which is why the 0.11.3 floor exists); runtime validation of the values happens inside `computer()`.
