...
|---|---|---|
| `coreouto[all]` | 0.11.2 | Agent loop, providers, tool registry, hooks (the runtime). 0.11.2 required: its openai provider uses low-level `create(stream=True)` instead of the SDK's `.stream()` events helper — the helper hard-rejects non-`strict` tools and breaks every openai-format provider on 0.11.1 (its `__version__` constant is stale and reports 0.11.1 — trust the dist-info). 0.10.0+ required for provider-level `error_handling` (see `core/error_rules.py`); 0.10.0 removed `retry_intervals`/`ON_RETRY` (never used here). Provider-level `stream=True` (see `core/providers.py:_instantiate`) requires 0.9.0+. 0.11.0+ required for the provider constructor `timeout` param, `ErrorRule.exc_type` matching + the `TIMEOUT_ERRORS` preset, and the contrib tracker hooks (`activity_tracker_hook`/`loop_progress_hook`) the stall watchdog is built on (`examples/27_wakeup.py`). 0.11.1+ required for optional `user_message` — `call(history=...)` alone continues a transcript (`examples/28_resume_interrupted.py`), which is how the watchdog restarts without injecting an artificial user message. |
